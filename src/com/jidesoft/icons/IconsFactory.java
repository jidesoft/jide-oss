 ...
 * <p/>
 * Any application usually need to access image files. One way to do it is to put those image files
 * in the installation and access them use direct file access. However this is not a good way
 * because you have to know the full path to the image file. So a better way that most Java
 * applications take is to bundle the image files with in the jar and use class loader to load
 * them.
 * <p/>
 * For example, if a class Foo needs to access image files foo.gif and bar.png, we put the image
 * files right below the source code under icons subfolder. See an example directory structure
 * below.
 * <pre>
 ...
 *                      /icons/bar.png
 * </pre>
 * Notes:<i> In IntelliJ IDEA's "Compile" tab of "Project Property" dialog, there is a way to set
 * "Resource Pattern". Here is the setting on my computer - "?*.properties;?*.xml;?*.html;?*.tree;?*.gif;?*.png;?*.jpeg;?*.jpg;?*.vm;?*.xsd;?*.ilayout;?*.gz;?*.txt"
 * for your reference. If so, all your images will get copies automatically to class output folder.
 * Although I haven't tried, I believe most Java IDEs have the same or similar feature. This feature
 * will make the usage of IconsFactory much easier. </i>
 * <p/>
 * If you setup directory structure as above, you can now use IconsFactory to access the images like
 * this.
 * <pre><code>
 * ImageIcon icon = IconsFactory.get(Foo.class, "icons/foo.gif");
 * </code></pre>
 * IconsFactory will cache the icon for you. So next time if you get the same icon, it will get from
 * cache instead of reading from disk again.
 * <p/>
 * There are a few methods on IconsFactory to create difference variation from the original icon.
 * For example, {@link #getDisabledImageIcon(Class,String)} will get the imaage icon with disabled
 * effect.
 * <p/>
 * We also suggest you to use the template below to create a number of IconsFactory classes in your
 * application. The idea is that you should have one for each functional area so that all your image
 * files can be grouped into each functional area. All images used in that functional area should be
 * put under the folder where this IconsFactory is. Here is an template.
 * <pre><code>
 ...
 * }
 * </code></pre>
 * In your own IconsFactory, you can further divide images into different groups. The example above
 * has two groups. There is also a convenient method getImageIcon() which takes just the icon name.
 * <p/>
 * In the template, we defined the image names as constants. When you have a lot of images, it's
 * hard to remember all of them when writing code. If using the IconsFactory above, you can use
 * <pre><code>
 * ImageIcon icon = TemplateIconsFactory.getImageIcon(TemplateIconsFactory.Group1.IMAGE1);
 * </code></pre>
 * without saying the actual image file name. With the help of intelli-sense (or code completion)
 * feature in most Java IDE, you will find it is much easier to find the icons you want. You can
 * refer to JIDE Components Developer Guide to see a screenshot of what it looks like in IntelliJ
 * IDEA.
 * <p/>
 * You probably also notice this is a main() method in this template. You can run it. When you run,
 * you will see a message printed out like this.
 * <pre><code>
 ...
 * </code></pre>
 * if you follow the instrcution and copy the html file to the same location as the source code and
 * open the html, you will see the all image files defined in this IconsFactory are listed nicely in
 * the page.
 */
public class IconsFactory {
     ...
     * <p/>
     * Please note, getImageIcon will print out error message to stderr if image is not found. The
     * reason we did so is because we want you to make sure all image files are there in your
     * application. If you ever see the error message, you should correct it before shipping the
     * product. But if you just want to test if the image file is there, you don't want any error
     * message print out. If so, you can use {@link #findImageIcon(Class,String)} method. It will
     * throw IOException when image is not found.
     *
    ...
    /**
     * Gets a brighter ImageIcon by passing class, a relative image file path and a percentage of
     * brightness.
     *
     * @param clazz    the Class<?>
     * @param fileName relative file name
    ...
    /**
     * Creates a gray version from an input ImageIcon. Usually gray icon indicates disabled. If
    ...
    /**
     * Creates a brighter image from an input image with a given percentage of brightness. If input
     * image is null, a blank ImageIcon will be returned.
     *
     * @param image   image
     * @param percent percentage of brightness
    ...
    /**
     * Creates a gray version from an input image with a given percentage of brightness. Usually
     * gray icon indicates disabled. If input icon is null, a blank ImageIcon will be returned.
     *
     * @param c       The component to get properties useful for painting, e.g. the foreground or
     *                background color.
     * @param icon    icon
    ...
    /**
     * Creates a negative version from an input black image which basically replaces black pixel
     * with white pixel.
     *
     * @param c    The component to get properties useful for painting, e.g. the foreground or
     *             background color.
...
//                writer.write("<td align=\"left\"><font face=\"Verdana\"><a href=\"" + prefix + ".getImageIcon("+ prefix + "." + name + "\">" + prefix + "." + name  + "<a></font></td>\n");
    ...
    /**
     * Gets part of the image from input image icon. It bascially takes a snapshot of the input
     * image at {x, y} location and the size is width x height.
     *
     * @param c      the component where the returned icon will be used. The component is used as
     *               the ImageObserver. It could be null.
     * @param icon   the original icon. This is the larger icon where a sub-image will be created
     *               using this method.
     ...
     * @param width  the width of the sub-image. It should be less than the width of the original
     *               icon.
    ...
    /**
     * Gets part of the image from input image icon. It bascially takes a snapshot of the input
     * image at {x, y} location and the size is width x height, then resize it to a size of
     * destWidth x destHeight.
     *
     * @param c          the component where the returned icon will be used. The component is used
     *                   as the ImageObserver. It could be null.
     * @param icon       the original icon. This is the larger icon where a sub-image will be
     *                   created using this method.
     * @param x          the x location of the sub-image, relative to the original icon.
     * @param y          the y location of the sub-image, relative to the original icon.
     * @param width      the width of the sub-image. It should be less than the width of the
     *                   original icon.
     * @param height     the height of the sub-image. It should be less than the height of the
     *                   original icon.
     * @param destWidth  the width of the returned icon. The sub-image will be resize if the
     ...
     * @param c         the component where the returned icon will be used. The component is used as
     *                  the ImageObserver. It could be null.
     * @param icon      the original icon. This is the larger icon where a small icon will be
     *                  created using this method.
     ...
     * @param width     the width of the smaller icon. It should be less than the width of the
     *                  original icon.
     * @param height    the height of the smaller icon. It should be less than the height of the
     *                  original icon.
     ...
     * Gets part of the image from input image icon. It bascially takes a snapshot of the input
     * image at {x, y} location and the size is width x height, then resize it to a size of
     * destWidth x destHeight. if the original icon is null or the specified location is outside the
     * original icon, EMPTY_ICON will be returned.
     *
     * @param c          the component where the returned icon will be used. The component is used
     *                   as the ImageObserver. It could be null.
     * @param icon       the original icon. This is the larger icon where a sub-image will be
     ...
     * @param location    the location as defined in SwingConstants - CENTER, NORTH, SOUTH, WEST,
     *                    EAST, NORTH_EAST, NORTH_WEST, SOUTH_WEST and SOUTH_EAST.
     * @param insets      the insets to the border. This parameter has no effect if the location is
     ...
     * @param icon        the original icon
     * @param overlayIcon the overlay icon.
     * @param x           the x location relative to the original icon where the overlayIcon will be
     *                    pained.
