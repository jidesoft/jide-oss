 ...
 * It provides a consistent means for creating custom tooltips without the need for
 * overriding JIDE components that create tooltips
 */
public class JToolTipFactory {

  private static JToolTipFactory tooltipFactory;
  ...
  /**
   * Creates a new tooltip. If overlapping is true then
   * the tooltip will take on the foreground/bakground color and
  ...
  /**
   * Sets the <code>JToolTipFactory</code> that will be used to obtain
   * <code>JToolTip</code>s.
  ...
  /**
   * Returns the shared <code>JToolTipFactory</code> which can be used
   * to obtain <code>JToolTip</code>s.
   *
   * @return the shared factory
   */
  public static JToolTipFactory getSharedInstance() {
    if (tooltipFactory == null) {
      tooltipFactory = new JToolTipFactory();
    }
