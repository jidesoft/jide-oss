...
/**
 * <code>StyledLabel</code> is a special JLabel which can display text in different styles.
 ...
 * and StyledLabel.
 * HTML JLabel is also buggy. See bug report at <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4373575">http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4373575</a>.
 ...
 * <p/>
 * StyledLabel sits between JLabel and JTextPane and provides a very simple and fast way to
 * display text in different color and style. It can also support decorations using all kinds of line styles.
 * <p/>
 * All the methods on JLabel still work as before. The methods added in StyledLabel are several methods for StyleRange, such as
 * {@link #setStyleRange(StyleRange)}, {@link #setStyleRanges(StyleRange[])}, {@link #clearStyleRange(StyleRange)}, and {@link #clearStyleRanges()}.
 * <p/>
 * This is one thing about StyleRange that you should be aware of, which could be considered as
 * a future enhancement item, is that the StyleRanges can't overlap with each other.
 * For example, if you defined a StyleRange that covers from index 0 to index 3,
 * you can't define any other StyleRange that overlaps with the first one.
 * If you do so, the second StyleRange will be ignored.
 * <p/>
 * We borrowed some ideas from SWT's StyledText when we designed StyledLabel, especially StyleRange concept.
 * Saying that, the features of the two components are not exactly the same since the purpose of the
...
public class StyledLabel extends JLabel {
    ...
    public void addStyleRange(StyleRange styleRange) {
        ...
        List ranges = internalGetStyleRanges();
    ...
    /**
     * @deprecated use {@link #addStyleRange(StyleRange)} instead. The method name "set" usually means
     *             clear all previous style range and set this as the new one, which is not correct.
     *             So using method name "add" makes more sense.
     */
    ...
    public void addStyleRanges(StyleRange[] styleRanges) {
        if (styleRanges != null) {
            for (int i = 0; i < styleRanges.length; i++) {
                internalGetStyleRanges().add(styleRanges[i]);
            }
    ...
    public StyleRange[] getStyleRanges() {
        List list = internalGetStyleRanges();
    ...
    /**
     * StyleRange could define color for the text and lines. However
     * when StyledLabel is used in cell renderer, the color could be conflict with
     * selection color. So usually when it is used as cell renderer, the color defined in StyleRange should be
     * ignored when cell is selected. If so, the foreground is used to paint all text and lines.
     *
     ...
     * This flag is used when StyledLabel is used as a selected cell renderer.
     * Since the selection usually has it own unique selection background and foreground,
     * the color setting set on this StyledLabel could be unreadable on the selection background,
     * it'd better if we don't use any color settings in this case.
     *
